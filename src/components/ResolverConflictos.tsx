...
export default function ResolverConflictos() {
  ...
  const conflictos = facturas.filter((f) => f.matchStatus === 'conflicto' || f.matchStatus === 'sin_match');

  const gastosPendientes = gastos.filter((g) => g.estado !== 'verificado' && g.estado !== 'sin_factura');
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-gray-500">
        {conflictos.length} factura{conflictos.length !== 1 ? 's' : ''} pendiente{conflictos.length !== 1 ? 's' : ''} de
        asignación
      </p>

      {conflictos.map((factura) => {
        const gastoActual = gastos.find((g) => g.id === factura.gastoId);
        const candidatos: Gasto[] = factura.candidatos
          ? factura.candidatos.map((id) => gastos.find((g) => g.id === id)).filter(Boolean) as Gasto[]
          : [];
            ...
            <div className="flex flex-col md:flex-row">
              ...
              <div className="flex-1 p-4 space-y-2 text-sm">
                <div className="flex items-start justify-between">
                  ...
                  <span
                    className={`text-xs font-medium px-2 py-1 rounded-full ${factura.matchStatus === 'conflicto' ? 'bg-orange-100 text-orange-700' : 'bg-yellow-100 text-yellow-700'}`}
                  >
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-xs text-gray-500">
                          {formatFecha(gastoActual.fecha)} · S/ {gastoActual.monto.toFixed(2)}
                        </p>
                      </div>
                      <button
                        onClick={() => asignarFactura(factura.id, gastoActual.id)}
                      ...
                      {candidatos.map((g) => (
                        <div key={g.id} className="flex items-center justify-between bg-gray-50 rounded-lg p-2.5">
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-gray-500">
                              {formatFecha(g.fecha)} · S/ {g.monto.toFixed(2)}
                            </p>
                          </div>
                          <button
                ...
                <div className="mt-2 pt-2 border-t border-gray-100">
                  ...
                  {busqueda[factura.id] && (
                    <div className="mt-2 space-y-1 max-h-32 overflow-y-auto">
                      {gastosPendientes
                        .filter((g) => g.descripcion.toLowerCase().includes(busqueda[factura.id].toLowerCase()))
                        .slice(0, 5)
                        .map((g) => (
                ...
                <button
                  onClick={() => {
                    if (factura.gastoId) actualizarEstado(factura.gastoId, 'sin_factura');
                  }}
