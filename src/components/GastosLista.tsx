...
import type { Gasto, GastoEstado, GastosListaProps } from '../types';

export default function GastosLista({ gastos, onAdjuntarFactura, onVerFactura, onEliminar }: GastosListaProps) {
    ...
    <div className="space-y-3">
          ...
          <tbody className="divide-y divide-gray-100">
            {gastos.map((gasto: Gasto) => (
              <tr key={gasto.id} className="hover:bg-gray-50">
                ...
                <td
                  className={`px-4 py-3 text-sm font-semibold text-right ${gasto.tipo === 'gasto' ? 'text-red-600' : 'text-green-600'}`}
                >
      ...
      <div className="md:hidden space-y-2.5">
        {gastos.map((gasto: Gasto) => (
          <div
            key={gasto.id}
            className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden active:bg-gray-50 transition-colors"
          >
            <div className="p-3.5">
            ...
            <div className="border-t border-gray-100 bg-gray-50/50 flex">
              ...
              {onEliminar && (
                <button
                  ...
                  className="px-3 flex items-center text-gray-400 hover:text-red-500 active:text-red-600 transition-colors border-l border-gray-100"
                  title="Eliminar"
                >
                  <Trash2 size={16} />
                </button>
...
function getEstadoBadge(estado: GastoEstado) {
  const styles: Record<GastoEstado, string> = {
  ...
  const icons: Record<GastoEstado, React.ComponentType<{ size?: number }> | null> = {
    pendiente: null,
  ...
  const labels: Record<GastoEstado, string> = {
    pendiente: 'Pendiente',
