          ...
          <div key={factura.id} className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            <div className="flex flex-col md:flex-row">
              <div className="md:w-40 shrink-0 bg-gray-50 border-b md:border-b-0 md:border-r border-gray-200">
                <img src={`data:${factura.imageMime};base64,${factura.imageBase64}`} alt="Factura" className="w-full h-40 md:h-full object-cover" />
              </div>
              ...
              <div className="flex-1 p-4 space-y-2 text-sm">
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    {factura.ocrData?.fecha && <p className="text-gray-500">Fecha: {factura.ocrData.fecha}</p>}
                    {factura.ocrData?.monto && <p className="text-gray-500">Monto: S/ {factura.ocrData.monto.toFixed(2)}</p>}
                    {factura.ocrData?.ruc && <p className="text-gray-500">RUC: {factura.ocrData.ruc}</p>}
                  </div>
                  <span className={`text-xs font-medium px-2 py-1 rounded-full ${factura.matchStatus === 'conflicto' ? 'bg-orange-100 text-orange-700' : 'bg-yellow-100 text-yellow-700'}`}>
                  ...
                  <div className="bg-blue-50 rounded-lg p-2.5 mt-2">
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-xs text-gray-500">{formatFecha(gastoActual.fecha)} · S/ {gastoActual.monto.toFixed(2)}</p>
                      </div>
                      <button onClick={() => asignarFactura(factura.id, gastoActual.id)}
                      ...
                      {candidatos.map((g) => (
                        <div key={g.id} className="flex items-center justify-between bg-gray-50 rounded-lg p-2.5">
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-gray-500">{formatFecha(g.fecha)} · S/ {g.monto.toFixed(2)}</p>
                          </div>
                          <button onClick={() => asignarFactura(factura.id, g.id)}
                ...
                <div className="mt-2 pt-2 border-t border-gray-100">
                  <div className="relative">
                    ...
                    <input type="text" placeholder="Buscar otro gasto..."
                      value={busqueda[factura.id] || ''}
                      onChange={(e) => setBusqueda((prev) => ({ ...prev, [factura.id]: e.target.value }))}
                  ...
                  {busqueda[factura.id] && (
                    <div className="mt-2 space-y-1 max-h-32 overflow-y-auto">
                      {gastosPendientes.filter((g) => g.descripcion.toLowerCase().includes(busqueda[factura.id].toLowerCase())).slice(0, 5).map((g) => (
                        <div key={g.id} className="flex items-center justify-between bg-gray-50 rounded-lg p-2">
                          ...
                          <button onClick={() => asignarFactura(factura.id, g.id)}
                            className="ml-2 bg-blue-600 text-white px-2 py-1 rounded text-xs hover:bg-blue-700">Asignar</button>
                        </div>
                      ))}
                    </div>
                  )}
                ...
                <button onClick={() => { if (factura.gastoId) actualizarEstado(factura.gastoId, 'sin_factura'); }}
