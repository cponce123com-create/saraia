...
import { Camera, Image as ImageIcon, X, Loader2, Check, AlertTriangle, Edit3 as Edit3Icon } from 'lucide-react';
...
export default function Escanear() {
  ...
  function handleFiles(fileList) {
    const nuevos = Array.from(fileList).map((file, i) => ({
      id: Date.now() + i, file, mime: file.type || 'image/jpeg', preview: URL.createObjectURL(file), b64: null,
    }));
    ...
    for (const img of images) {
      ...
      if (API_KEY) {
        try {
          ...
          const response = await fetch('https://api.deepseek.com/chat/completions', {
            ...
            body: JSON.stringify({
              ...
              messages: [{
                role: 'user',
                content: [
                  { type: 'text', text: 'Extract invoice data from this image. Return ONLY JSON: {"fecha":"YYYY-MM-DD"|null,"monto":number|null,"proveedor":string|null,"ruc":string|null}' },
          ...
          if (jsonMatch) ocrData = JSON.parse(jsonMatch[0]);
        } catch (err) {
      ...
      resultsArr.push({
        imagen: img, ocr: ocrData, match: matchResult,
        status: ocrData ? status : 'manual', gastoAsignado, error: errorMsg, b64,
      });
    }
  ...
  function asignarDatosManuales(idx, datos) {
    const r = results[idx];
    if (!r) return;
    ...
    if (matchResult.match === 'unico') {
      ...
      gastoAsignado = matchResult.gastos[0];
      adjuntarFactura(gastoAsignado.id, {
        imageBase64: r.b64, imageMime: r.imagen.mime, ocrData: datos,
        matchStatus: 'manual', createdAt: new Date().toISOString(),
      });
      actualizarEstado(gastoAsignado.id, 'verificado');
    }
    setResults((prev) => {
    ...
    <div className="space-y-6">
      ...
      {!API_KEY && (
        <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4 text-sm text-yellow-800">
          <strong>API Key no configurada.</strong> Las facturas se procesaran en modo manual.
          Para activar OCR, agrega VITE_DEEPSEEK_API_KEY en Render.
        </div>
      )}
          ...
          {images.length > 0 && (
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4">
                ...
                {images.map((img) => (
                  <div key={img.id} className="relative group aspect-square rounded-lg overflow-hidden bg-gray-100 border">
                    ...
                    <button onClick={() => removeImage(img.id)}
                      className="absolute top-1 right-1 bg-black/60 text-white rounded-full p-1 opacity-0 group-hover:opacity-100"><X size={14} /></button>
                  </div>
                ))}
              </div>
              <button onClick={procesarTodo} disabled={processing}
                ...
                {processing ? 'Procesando...' : 'PROCESAR ' + images.length + ' FOTO' + (images.length !== 1 ? 'S' : '')}
              </button>
            </div>
          )}
        </>
          ...
          {results.map((r, idx) => (
            <div key={r.imagen.id} className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
              <div className="flex flex-col sm:flex-row">
                <div className="sm:w-32 sm:h-32 shrink-0 bg-gray-100 cursor-pointer border-b sm:border-b-0 sm:border-r"
                ...
                <div className="flex-1 p-3 sm:p-4 text-sm min-w-0">
                  <div className="flex items-start justify-between mb-2">
                    <span className={'text-xs font-medium px-2 py-0.5 rounded-full ' + (
                      ...
                      {r.status === 'auto' ? 'Asignado automatico' :
                       r.status === 'conflicto' ? 'Conflicto' : 'Pendiente manual'}
                    </span>
                    ...
                    <div className="space-y-0.5">
                      ...
                      {r.gastoAsignado && (
                        <div className="mt-1 pt-1 border-t text-xs">
                          <p className="text-gray-500">Asignado a: <strong className="text-blue-700">{r.gastoAsignado.descripcion}</strong> - S/ {r.gastoAsignado.monto.toFixed(2)}</p>
                        </div>
                      )}
                    </div>
                  ) : (
                    <FormManual onSubmit={(datos) => {
                      setProcessing(true);
                  ...
                  {r.status === 'conflicto' && r.match?.gastos?.slice(0, 3).map((g) => (
                    <div key={g.id} className="flex items-center justify-between text-xs mt-1">
                      <span className="truncate">{g.descripcion} - S/ {g.monto.toFixed(2)}</span>
                      <button onClick={() => {
                      ...
                      }} className="ml-2 bg-blue-600 text-white px-2 py-0.5 rounded text-xs">Asignar</button>
                    </div>
                  ))}
                </div>
              </div>
...
function FormManual({ onSubmit }) {
  const [form, setForm] = useState({ proveedor: '', fecha: '', monto: '', ruc: '' });
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!form.proveedor || !form.monto) { toast.error('Proveedor y Monto son obligatorios'); return; }
    onSubmit({
      proveedor: form.proveedor || null, fecha: form.fecha || null,
      monto: form.monto ? parseFloat(form.monto) : null, ruc: form.ruc || null,
      tipo_comprobante: 'factura', numero_comprobante: null,
    });
  };
    ...
    <form onSubmit={handleSubmit} className="space-y-3 mt-2 bg-blue-50 rounded-lg p-3 border border-blue-200">
      ...
      <div className="grid grid-cols-2 gap-2">
        <input value={form.proveedor} onChange={(e) => setForm((f) => ({ ...f, proveedor: e.target.value }))}
          placeholder="Proveedor *" className="col-span-2 border rounded-lg px-3 py-2 text-sm" required />
        <input type="date" value={form.fecha} onChange={(e) => setForm((f) => ({ ...f, fecha: e.target.value }))}
          placeholder="Fecha" className="border rounded-lg px-3 py-2 text-sm" />
        <input type="number" step="0.01" value={form.monto} onChange={(e) => setForm((f) => ({ ...f, monto: e.target.value }))}
          placeholder="Monto S/ *" className="border rounded-lg px-3 py-2 text-sm" required />
      </div>
      <input value={form.ruc} onChange={(e) => setForm((f) => ({ ...f, ruc: e.target.value }))}
        placeholder="RUC (opcional)" className="w-full border rounded-lg px-3 py-2 text-sm" />
